...
function mount(): void {
  ...
  try {
    ...
    console.error("[LWmark] 界面挂载失败：", e);
    const el = document.getElementById("app");
    if (el) {
      el.innerHTML =
        '<div style="font:13px/1.7 system-ui;padding:32px;color:#333">' +
        "<b>LWmark 启动失败</b><br>界面没能挂载，详情见开发者控制台。<br>" +
...
async function boot() {
  ...
  try {
    ...
    console.warn("[LWmark] Neutralino 初始化失败：", e);
  }
  ...
  try {
    ...
    console.warn("[LWmark] 配置初始化失败，本次会话用默认值：", e);
  }
  ...
  if (inNL) {
    try {
      ...
      console.warn("[LWmark] 窗口尺寸恢复失败：", e);
    }
  }
